...
<body>
	<div class="container">
				...
				<div class="border border-dark m-2">
					...
					<div class="row m-3">
						<div class="  col-6 achats  ">
							<label for="achats">Achats</label> <input type="radio"
								id="achats" name="choix" checked value="achats" />
							<div class="p-2">
						...
						<div class="col col-6 ventes ">
							...
							<div class="p-2">
								<input type="checkbox" id="venteEnCours" name="triVentes"
									...
									id="ventesNonDebutees" name="triVentes"
									value="ventesNonDebutees" /> <label for="ventesNonDebutees">ventes
									non débutées</label><br> <input type="checkbox"
									id="ventesTerminees" name="triVentes" value="ventesTerminees" />
					...
					<div class="card col-auto " style="width: 18rem;">
						...
						<ul class="list-group list-group-flush ">
							<li class="list-group-item"><a
								href="<%=request.getContextPath()%>/afficherVente?idArticle=<%=article.getNoArticle()%>"><%=article.getNomArticle()%></a>
							</li>
							<li class="list-group-item">Prix : <%=article.getPrixVentes()%>
								points
							</li>
							<li class="list-group-item">Fin date enchère : <%=article.getDateFinEncheres()%></li>
							<li class="list-group-item">Vendeur : <a
								href="<%=request.getContextPath()%>/afficherProfil?id=<%=article.getVendeur().getNoUtilisateur()%>"><%=article.getVendeur().getPseudo()%></a>
							</li>
						</ul>
		...
		<script>
			...
			const achat = document.getElementById("achats");
			achat.addEventListener('click', event => {
			  	applyAchat();
			});
			
			
			const vente = document.getElementById("ventes");
			vente.addEventListener('click', event => {
				applyVente();
			});
			
			
			function applyAchat() {
				  const checkbox_1 = document.getElementById("encheresOuvertes");
				  const checkbox_2 = document.getElementById("encheresEnCours");
				  const checkbox_3 = document.getElementById("encheresRemportees");
				  const checkbox_4 = document.getElementById("venteEnCours");
				  const checkbox_5 = document.getElementById("ventesNonDebutees");
				  const checkbox_6 = document.getElementById("ventesTerminees");
				  
				  checkbox_1.disabled = false;
				  checkbox_2.disabled = false;
				  checkbox_3.disabled = false;
				  checkbox_4.disabled = true;
				  checkbox_5.disabled = true;
				  checkbox_6.disabled = true;
			...
			function applyVente() {
				  ...
				  checkbox_5.disabled = false;
				  checkbox_6.disabled = false;
